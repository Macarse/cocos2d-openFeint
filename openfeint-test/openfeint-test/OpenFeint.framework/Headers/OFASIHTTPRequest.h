...
typedef enum _OFASIAuthenticationState {
	OFASINoAuthenticationNeededYet = 0,
	OFASIHTTPAuthenticationNeeded = 1,
	OFASIProxyAuthenticationNeeded = 2
} OFASIAuthenticationState;

typedef enum _OFASINetworkErrorType {
    OFASIConnectionFailureErrorType = 1,
    OFASIRequestTimedOutErrorType = 2,
    OFASIAuthenticationErrorType = 3,
    OFASIRequestCancelledErrorType = 4,
    OFASIUnableToCreateRequestErrorType = 5,
    OFASIInternalErrorWhileBuildingRequestType  = 6,
    OFASIInternalErrorWhileApplyingCredentialsType  = 7,
	OFASIFileManagementError = 8,
	OFASITooMuchRedirectionErrorType = 9,
	OFASIUnhandledExceptionError = 10
	
} OFASINetworkErrorType;
...
@interface OFASIHTTPRequest : NSOperation <NSCopying> {
	...
	// If error code is = OFASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	// See OFASIAuthenticationState values above. 0 == default == No authentication needed yet
	OFASIAuthenticationState authenticationNeeded;
...
// Used internally to record bandwidth use, and by OFASIInputStreams when uploading. It's probably best if you don't mess with this.
...
@property (assign, readonly) OFASIAuthenticationState authenticationNeeded;
